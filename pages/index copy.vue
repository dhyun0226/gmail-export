  ...
  <div class="min-h-screen bg-background text-text-DEFAULT font-sans">
    <div class="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <div v-if="!isAuthenticated" class="max-w-md mx-auto text-center bg-white p-8 rounded-xl shadow-lg">
        <p class="mb-8 text-lg text-text-light">구글 계정으로 로그인하여 메일을 조회하고 엑셀로 다운로드하세요.</p>
        <button
          @click="login"
      ...
      <div v-else class="max-w-full mx-auto">
        ...
        <div v-if="emails.length > 0" class="bg-white rounded-xl shadow-lg">
          <div class="p-8 border-b border-gray-200 flex justify-between items-center">
            <h2 class="text-2xl font-bold text-primary-dark">조회 결과: <span class="text-accent-DEFAULT">{{ emails.length }}</span>개</h2>
            <button
              @click="downloadExcel"
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="(email, index) in emailsWithRowspan" :key="`${email.id}-${index}`" class="hover:bg-gray-50 transition duration-150 ease-in-out even:bg-gray-50">
                  <td 
                  ...
                  <td class="px-6 py-4 text-sm text-text-DEFAULT">
                    <div class="flex items-center gap-2">
                      ...
                      <span v-if="email.unipassData" class="text-accent-DEFAULT text-xs">●</span>
                    </div>
                  </td>
                  <td class="px-6 py-4 text-sm text-text-DEFAULT">{{ email.trackingNumber || 'N/A' }}</td>
                  <td class="px-6 py-4 text-sm text-text-light">{{ email.acceptanceTime || '-' }}</td>
                  <td class="px-6 py-4 text-sm text-text-light">{{ email.clearanceTime || '-' }}</td>
                  <td class="px-6 py-4 text-sm text-text-light">{{ email.date }}</td>
                  <td class="px-6 py-4 text-sm text-text-light">{{ email.time }}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <!-- 더보기 버튼 -->
          <div v-if="emails.length > displayCount" class="p-6 text-center border-t border-gray-200 bg-gray-50">
            <button
              @click="showMore"
        ...
        <!-- 에러 메시지 -->
        <div v-if="error" class="bg-red-50 border border-red-200 text-red-700 px-6 py-4 rounded-md shadow-md mt-8">
          {{ error }}
        </div>
      </div>
    </div>
